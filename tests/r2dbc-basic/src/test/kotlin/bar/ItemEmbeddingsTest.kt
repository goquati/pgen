...
import de.quati.pgen.tests.r2dbc.basic.generated.db.bar.public.ItemEmbeddings
...
class ItemEmbeddingsTest {
    ...
    @Test
    fun `create item embedding with defaults for item_uuid and created_at`(): Unit = runBlocking {
    ...
    @Test
    fun `update item embedding using UpdateEntity updates only specified columns`(): Unit = runBlocking {
    ...
    @Test
    fun `delete item embedding removes row`(): Unit = runBlocking {
    ...
    @Test
    fun `constraint error - title must not be empty or whitespace`(): Unit = runBlocking {
    ...
    @Test
    fun `test cosine distance and similarity`(): Unit = runBlocking {
        ...
        val result = db.suspendTransaction {
